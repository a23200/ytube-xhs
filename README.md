...
- 每个平台会生成 `*-quality-report.json`，记录估算改写程度、最长重复片段、重写次数和数据转换证据。70% 是可解释的文本改写目标，不是版权或平台原创认证。
...
所有平台正文必须通过无小标题、100 字钩子、来源数据和改写程度校验。不合格时会把具体命中项反馈给 LLM 定向重写，不会简单删除标题、截断正文或改成本地模板。
