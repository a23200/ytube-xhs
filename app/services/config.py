...
class Settings:
    ...
    def __init__(self) -> None:
        ...
        self.llm_base_url = os.getenv("XHS_LLM_BASE_URL", "https://api.openai.com/v1").rstrip("/")
        self.llm_model = os.getenv("XHS_LLM_MODEL", "gpt-4o-mini")
        self.llm_requires_api_key = _llm_requires_api_key(self.llm_base_url)
        self.llm_timeout_ms = _int_env("XHS_LLM_TIMEOUT_MS", 60000)
        self.llm_max_chars = _int_env("XHS_LLM_MAX_CHARS", 60000)
        self.llm_max_tokens = _int_env("XHS_LLM_MAX_TOKENS", 1200)
